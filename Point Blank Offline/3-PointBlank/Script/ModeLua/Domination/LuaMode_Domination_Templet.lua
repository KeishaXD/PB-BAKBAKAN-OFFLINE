...
function	OnPreStartRound_Templet()
end

-- 라운드 시작했을 때 호출되는 함수
function	OnStartRound_Templet()
end

-- 라운드 진행 중 상태 확인하는 함수
function	OnBattleUpdate_Templet()
end

-- 라운드를 종료했을 때 호출되는 함수
function	OnEndRound_Templet()
end
...
function	OnTimeOut_Templet( timerIdx )
end

-- 오브젝트 설치 요청시 호출되는 함수
-- 설치를 허용하려면 true, 불허하려면 false 리턴
function	OnInstall_Templet( SlotIdx, Weapon )
	return false;
end

-- 오브젝트 해체를 요청시 호출되는 함수
-- 해체를 허용하려면 true, 불허하려면 false 리턴
function	OnUninstall_Templet( SlotIdx, Weapon )
	return false;
end

-- 오브젝트 사용을 요청시 호출되는 함수
-- 사용을 허용하려면 true, 불허하려면 false 리턴
function	OnUse_Templet( SlotIdx, Weapon )
	return false;
end

-- 게임 오브젝트에 데미지를 입혔을 경우 호출되는 함수
function	OnDamageObject_Templet( SlotIdx, ObjectIdx, Damage)
end

-- 게임 오브게트가 파괴됐을떄 호출되는 함수
function	OnDestroyObject_Templet( SlotIdx, ObjectIdx )
end

-- 트리거 영역에 들어갔을때 호출되는 함수
function	OnEnterTrigger_Templet( SlotType, SlotIdx, TriggerIdx )
end

-- 트리거 영역에서 나왔을떄 호출되는 함수
function	OnLeaveTrigger_Templet( SlotType, SlotIdx, TriggerIdx )
end

-- 특수키 처리 요청이 들어왔을 때 호출되는 함수
function	OnKeyProc_Templet( SlotIdx, Key )
	return EVENT_ERROR_FAIL;
end

-- 특정 그룹의 AI가 전멸했을때 호출되는 함수
function OnAnnihilateAIGroup_Templet( GroupName )
end

DAY_WAVE_FUNCTION_TEMPLET = {
