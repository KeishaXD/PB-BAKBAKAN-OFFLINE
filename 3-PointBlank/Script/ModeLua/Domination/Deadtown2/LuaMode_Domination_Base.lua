...
require "ModeLua.Domination.Deadtown2.LuaMode_Domination_D6P"
require "ModeLua.Domination.Deadtown2.LuaMode_Domination_D6W1"

-- {Day No., Wave No., Goal object, Time, KillCount}
-- unused column is -1
DAY_WAVE_LIST = {
	-- Day 1
	{ 6, 0, GOAL_OBJEcT.DAY_PROLOGUE, 	10, -1 },
	{ 6, 1, GOAL_OBJEcT.BOSS,			-1,	-1 },
	{ -1, -1, -1 },	-- end mode
}

DAY_WAVE_FUNCTION = {
	DAY_WAVE_FUNCTION_D6P,
...
function	OnEndRound()
	...
	-- 다음 라운드 설정이 없으면 배틀 종료
 	if( DAY_WAVE_LIST[GetRoundCount()+1] == nil or
...
function	OnUninstall( SlotIdx, Weapon )
	if( SKILL_SET[SKILL["SUPPLYBASE"]][1][SKILL_ITEM] <= Weapon	and
		Weapon <= SKILL_SET[SKILL["SUPPLYBASE"]][4][SKILL_ITEM] ) then	-- 스킬 서플라이베이스
		return true;
	elseif( SKILL_SET[SKILL["DUMMY"]][1][SKILL_ITEM] <= Weapon and
			Weapon <= SKILL_SET[SKILL["DUMMY"]][4][SKILL_ITEM] ) then	-- 더미
		return true;
	elseif( SKILL_SET[SKILL["SENTRYGUN"]][1][SKILL_ITEM] <= Weapon and
			Weapon <= SKILL_SET[SKILL["SENTRYGUN"]][4][SKILL_ITEM] ) then	-- 더미
		return true;
	end
...
function	OnUse( SlotIdx, Weapon )
	if( SKILL_SET[SKILL["SUPPLYBASE"]][1][SKILL_ITEM] <= Weapon	and
		...
		-- 사용 효과
		if( Weapon == SKILL_SET[SKILL["SUPPLYBASE"]][1][SKILL_ITEM] ) then
			local Effects = {
		...
		elseif( Weapon == SKILL_SET[SKILL["SUPPLYBASE"]][2][SKILL_ITEM] ) then
			local Effects = {
				{EFFECT_TYPE.HP, nil, 30},
				{EFFECT_TYPE.AMMO, WEAPON_USAGE.PRIMARY, 100},
				{EFFECT_TYPE.AMMO, WEAPON_USAGE.SECONDARY, 100},
			...
			SetGameEffect(0, 4, Effects);
		elseif( Weapon == SKILL_SET[SKILL["SUPPLYBASE"]][3][SKILL_ITEM] ) then
			local Effects = {
				{EFFECT_TYPE.HP, nil, 30},
				{EFFECT_TYPE.AMMO, WEAPON_USAGE.PRIMARY, 100},
				{EFFECT_TYPE.AMMO, WEAPON_USAGE.SECONDARY, 100},
			...
			SetGameEffect(0, 4, Effects);
		elseif( Weapon == SKILL_SET[SKILL["SUPPLYBASE"]][4][SKILL_ITEM] ) then
			local Effects = {
				{EFFECT_TYPE.HP, nil, 30},
				{EFFECT_TYPE.AMMO, WEAPON_USAGE.PRIMARY, 100},
				{EFFECT_TYPE.AMMO, WEAPON_USAGE.SECONDARY, 100},
...
function	OnDamageObject( SlotIdx, ObjectIdx, Damage)
	...
	elseif( DAY_WAVE_FUNCTION[DayWaveCount][DAMAGEOBJECTFUNCTION] ~= nil ) then
		DAY_WAVE_FUNCTION[DayWaveCount][DAMAGEOBJECTFUNCTION]( SlotIdx, ObjectIdx, Damage);
	end
end
...
function	OnKeyProc( SlotIdx, Key )
	...
	UsedSkillItem[SlotIdx+1] = SKILL_SET[SkillIdx][SkillLevel][SKILL_ITEM]

	ScriptDebugMsg( "UsedSkillItem:" .. UsedSkillItem[SlotIdx+1] );

	if( SkillIdx == SKILL["SPECIAL"] ) then
		if( SkillLevel == 1 ) then
			local Effects = {
				{EFFECT_TYPE.DAMAGE, nil, 1},
			};
			SetGameEffect(SlotIdx, 1, Effects);
			DisableTime_PowerUp[SlotIdx] = GetGameTime() + SKILL_DAMAGEUP_DURATION;
		elseif( SkillLevel == 2 ) then
			local Effects = {
				{EFFECT_TYPE.SPEED, nil, 1},
			};
			SetGameEffect(SlotIdx, 1, Effects);
